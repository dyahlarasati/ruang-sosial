...
<body>
    ...
    <table style="margin-bottom: 10px; margin-top:50px;" cellpadding="5">
        <tbody>
            <tr>
                <th>Nama Panti</th>
                <td>:</td>
                <td>{{ $tempat_donasi->nama_tempat_donasi }}</td>
            </tr>
        </tbody>
    </table>
    ...
    <table style="text-align: center" border="1" cellspacing="0" cellpadding="8" width="100%">
        ...
        <tbody>
            @foreach ($info as $i)
            @foreach (App\AdminModel\AktivitasDonasi::with(['tempat_donasi','donasi'])
            ->where('id_aktivitas_donasi',$i->id_aktivitas_donasi)->withTrashed()->get() as $item)
            <tr>
